...
public class QuarterNoteAnim : MonoBehaviour {
	...
	public void DrawSharpAnimation() {
		...
		GameObject[] clients = GameObject.FindGameObjectsWithTag("QuarterNote");
...
//			Sprite clientSprite = spriteManager.AddSprite(client, 1f, 1f, new Vector2(0f, 2f/3f), new Vector2(1f/6f, 1f/3f), Vector3.zero, false);
			Sprite clientSprite = spriteManager.AddSprite(client, 1f, 1f, new Vector2(0f, 4f/15f), new Vector2(1f/18f, 1f/15f), Vector3.zero, false);
	...
	//		sharpAnim.BuildUVAnim(spriteManager.PixelCoordToUVCoord(new Vector2(0f, 0f)), spriteManager.PixelSpaceToUVSpace(new Vector2(125f, 125f)), 6, 3, 18, 24);
			anim.BuildUVAnim(new Vector2(0f, 4f/15f), new Vector2(1f/18f, 1f/15f), 18, 2, 36, 24);
			
			// Prepare sprite
			clientSprite.AddAnimation(anim);
			clientSprite.PlayAnim("QuarterNote Animation");
		}
	}
}
